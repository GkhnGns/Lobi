...
$TagSql = $conn->prepare("SELECT * FROM uyeetiketler WHERE seo = ?");  
...
$verisayisi = count($conn->query("SELECT * FROM uyeler WHERE anahtar LIKE '%".$etiket['baslik']."%'",PDO::FETCH_ASSOC)->fetchAll());
$gosterilecek = 10;
$sayfa;
...
$NewSql = $conn->prepare("SELECT * FROM uyeler WHERE anahtar LIKE ? ORDER BY id DESC LIMIT $baslangic,$gosterilecek");  
...
<div class="container">
<div class="col-md-8">	
<?php while($ilan = $NewSql->fetch()){ 
$kategoriSorgula = $conn->prepare("SELECT * FROM kategori WHERE id = ?");
$kategoriSorgula->execute(array($ilan['kategori']));
$kateqoriCek = $kategoriSorgula->fetch();
$ResimSorgu  	= $conn->prepare("select * from resimler where uye_id = ? order by id asc limit 1");
$ResimSorgu->execute(array($ilan['id']));
$UyeResimCek = $ResimSorgu->fetch(); 
$id 		= $UyeResimCek['id'];
...
<div class="media">
<a class="pull-left" href="ilan/<?php echo $ilan['seo']; ?>/<?php echo $ilan['id']; ?>">
<div class="cerceve"><img src="<?php $count = $ResimSorgu->rowcount();
if ($count > 0){ ?>modelresim/<?php echo $resim; }else{ echo "img/resimyok.png"; } ?>" alt="<?php echo $ilan['isim']; ?>" class="resim"></div>
</a>
<div class="panel-body">
<h4 class="media-heading"><a href="ilan/<?php echo $ilan['seo']; ?>/<?php echo $ilan['id']; ?>.html"><?php echo $ilan['isim']; ?></a> </h4>
<p class="detay"><?php $metin = mb_substr($ilan['aciklama'],0,350).'... '; echo strip_tags($metin);  ?></p>
</div>
</div>
</div>
<div class="panel-footer">
<ul class="list-inline list-unstyled">
<?php $IlanTel 	= $ilan['telefon']; $WhatsappYaz 	= substr($IlanTel,0,4)." ".substr($IlanTel,4,3)." ".substr($IlanTel,7,2)." ".substr($IlanTel,9,2); ?>
<a href="ilan/<?php echo $ilan['seo']; ?>/<?php echo $ilan['id']; ?>.html" class="btn btn-xs btn-success">İlan</a>            
<a href="tel:<?php echo $ilan['telefon']; ?>" class="btn btn-xs btn-default"><?php echo $WhatsappYaz; ?></a>
<a href="kategori/<?php echo $kateqoriCek['seo']; ?>/<?php echo $kateqoriCek['id']; ?>.html" class="btn btn-xs btn-default"><?php echo $kateqoriCek['baslik']; ?></a>
</ul>
</div>
</div>
<?php } ?>
...
<?php 
if($sayfa > 1){
echo '<li><a href="ilan-etiket/'.$tag.'.html?sayfa=1">İlk</a></li>';
echo '<li><a title="önceki" href="ilan-etiket/'.$tag.'.html?sayfa='.($sayfa-1).'">&laquo;</a></li>';
}
$ekgoster = 2;
...
echo '<li><a href="ilan-etiket/'.$tag.'.html?sayfa='.$i.'">'.$i.'</a></li>';
}
}
}
if($sayfa < $sayfasayisi){
echo '<li><a title="sonraki" href="ilan-etiket/'.$tag.'.html?sayfa='.($sayfa+1).'">&raquo;</a></li>';
echo '<li><a href="ilan-etiket/'.$tag.'.html?sayfa='.$sayfasayisi.'">Son</a></li>';
}
?>	
</ul>
</nav>	
